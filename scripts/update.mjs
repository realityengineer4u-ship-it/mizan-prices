...
async function egypt() {
  ...
  try {
    const all = [];
    for (const it of await news("سعر الأسمنت اليوم متوسط الطن")) { try { const cc = parseCement(text(await get(it.link))); log("cement article " + cc.length + " hits: " + it.link.slice(0, 90)); all.push(...cc); } catch (e) { log("skip", e.message); } }
